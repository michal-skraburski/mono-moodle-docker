...
% TODO (reachability): add a sentence here once the editing-form link is
% implemented, e.g. "The page is linked directly from the question editing
% form via ...".
...
% TODO: no personal data is collected by any of the changes. New state lives
% in sessionStorage and contains only layout preferences keyed by question
% id. The docs page validates its only input parameter against directory
% escape. Code execution remains isolated on the Jobe server.
% TODO: confirm and state that survey responses were collected anonymously.

\subsection{Verification \& Validation}

\subsubsection{Verification}
Automated verification rests on the two test harnesses of the development environment, each runnable with a single script. For the documentation page, PHPUnit tests were written. \texttt{docs.php} is a plain script rather than a class, so there is nothing to call directly: the tests instead fake the request by setting \texttt{\$\_GET} (which \texttt{optional\_param()} reads) and include the script inside an output buffer, capturing whatever it renders. Three tests cover the page: a real document renders; a nonexistent page raises an exception; and a traversal attempt requesting \texttt{../../version.php} is rejected. That last target was chosen with care: it is a real file outside the documentation directory, so the test cannot be satisfied by a naive existence check and instead attacks the path-containment defence directly.

For regression testing, the plugin's upstream Behat suite was run: browser-driven acceptance tests, executed against the Selenium container by \texttt{behat.sh}. Their evidential value comes from their provenance (tests not written by this project, exercising code it changed), so passing them is independent evidence that the test case indexing rewrite preserved existing behaviour. The suite also demonstrably earned its keep: it exposed that the one-blank test case defaults broke upstream scenarios, forcing the reset to three described under FR2. That is a concrete regression caught by the harness before any user saw it, and it justifies the effort spent making the suite runnable (DE3).

The remaining evidence is measured or manual. Lighthouse audits provide the quantitative check for NFR2, with the before and after scores (93\% and 97\%) recorded at the time of the change. The user-interface features (the layout switcher, the info panel collapse, the save draft button and the precheck fix) were verified manually, both directly and through the closed feedback loop of peers and supervisor described in Section~\ref{process}. The limitation is stated plainly: automated coverage is concentrated on the documentation page and the upstream suite, and the new UI behaviour has no automated tests of its own.

\subsubsection{Validation}
Validation against the requirements is summarised in the traceability table (Table~\ref{tab:traceability})~\cite{goteltraceability}, which traces each requirement from Section~\ref{process} to its implementation, the evidence that verified it, and its status. The statuses are kept honest rather than uniform: most requirements are met, FR2 is met partially with the trade-off argued in its implementation section, and two were deferred.

The deferred requirements were scoped out deliberately rather than silently dropped. FR5 (hints for hidden test cases) and FR9 (a copy button on code blocks) both originated in the user survey, and neither is abandoned: the Results section reframes the hint mechanism as the broader question of how much information hidden test cases should reveal, and identifies the copy button as future work with a high benefit-to-cost ratio. Deferring them was a scoping decision made against the time remaining, and the reasoning is recorded where the requirements came from, in the survey analysis, rather than asserted here.
...
FR3 & Help content stays visible & % TODO: implemented? verify

 & % TODO
 & % TODO: \statusmet / \statusdeferred
 \\
...
FR6 & Save work on demand & Save Draft button & Manual & \statusmet \\
